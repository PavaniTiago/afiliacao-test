...
A API utiliza **Better Auth** para autenticação. Todas as rotas protegidas requerem um token Bearer no header `Authorization`.
...
export class ApiClient {
  ...
  private async getAuthToken(): Promise<string | null> {
    // Implementar lógica para obter token da sessão
    // Exemplo com Better Auth:
